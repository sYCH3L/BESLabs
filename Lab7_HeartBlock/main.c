...
//The Ready output is connected to PF3
#define Ready                   (*((volatile unsigned long *)0x40025020))
//AS input is connected to PF4
#define AS                      (*((volatile unsigned long *)0x40025040))
//the VT output is connected to PF1
...
void Delay1ms(unsigned long msec){
	  unsigned long i;
  while(msec > 0){
    i = 13330;  // this number means 1ms
    while(i > 0){
      i -= 1;
    }
    msec -= 1; // decrements every  1ms
  }
}
void foo(void)
{
